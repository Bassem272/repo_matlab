...
capc(N)=cap(N);
 ...
 Fr1(N)=R(N)*100;
    fault(N)= F(N);
        fault2(N)=f(N);
            fault3(N)=ff(N);time(N)=fftime(N);
        
 simin(N) = Simulink.SimulationInput(mODL_NAME);
%    
%    simin(N)= simin(N).setBlockParameter('nota/test','ActivePower',num2str(react(N))) 
% simin(N)= simin(N).setBlockParameter('nota/test','InductivePower',num2str(indc(N))) 
% simin(N)= simin(N).setBlockParameter('nota/test','CapacitivePower',num2str(capc(N))) 
%  

simin(N)= simin(N).setBlockParameter('notch1/fault3','SwitchTimes',string(time(N)));

% 
%      simin(N)= simin(N).setBlockParameter('notch1/fault3','SwitchTimes',num2str(time(N))) 
...
% simin(N)= simin(N).setBlockParameter('nota/test','InductivePower',num2str(indc(N))) 
% simin(N)= simin(N).setBlockParameter('nota/test','CapacitivePower',num2str(capc(N))) 
%       simin(N)= simin(N).setBlockParameter('notch1/fault','SwitchTimes',num2str(time(N))) 
...
save('drago3.mat');%normal drago2 5 ohm without switch timegood results///drago3 timed and 100 ohm
